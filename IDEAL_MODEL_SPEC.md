...
### 3.1 The Projection Map $\Psi$ (Theoretical / Legacy LCP)

> ⚠️ **DEPRECATION NOTICE (v2.1)**
> The Linear Congruence Projection (LCP) described below represents the idealized theoretical model. In practice, it suffers from "Reduction Chaos" (discontinuous jumps in coefficients $a, b$ during Gaussian reduction).
>
...
**Linear Congruence Projection (Theoretical):**
We map the reduced form coefficients $(a, b)$ of the ideal class $S$ onto the finite field $\mathbb{Z}_p$. For the $k$-th step in the logical sequence generated from state $S$:
...
**Why LCP? (Theoretical justification)**
