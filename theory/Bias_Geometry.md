...
We resolve this by defining the Bias Channel using **Aitchison Geometry** (Compositional Data Analysis).

---

### 2. The Logit Space as Tangent Bundle
Let $\Delta^{V-1}$ be the probability simplex of actions.
The Logit Space $\mathcal{L} \cong \mathbb{R}^V$ is interpreted as the **Tangent Space** $T_p \Delta^{V-1}$ (modulo scaling invariance).
...
**Engineering Fix**: Because Softmax saturates ($J_\sigma \to 0$ for high-confidence predictions), this explains why VAPO uses **Perturbation (Derivative-Free)** rather than Gradient Descent. When the generator is confident but wrong (**Gradient Vanishing**), a random jump in $\vec{b}$ (Logit space) can escape the saturation plateau, whereas gradients would die.
