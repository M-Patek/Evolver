...
1.  **Feasibility**: Is action $a$ valid at state $s$? ($C \subseteq S \times A$)
2.  **Admissibility**: Is the sequence $a_1 \dots a_n$ valid? ($C \subseteq A^*$)
3.  **Invariance**: Does the transition preserve truth? ($T(s, a) \in S_{true}$)
...
This formalizes the constraint as a **Subshift of Finite Type** (或上下文相关语言，取决于 STP 的复杂度)。
